...
if [ -z "${YARD_RELEASES_USER:-}" ] || [ -z "${YARD_RELEASES_PWD:-}" ]; then
    echo "Ошибка: Необходимо установить переменные среды YARD_RELEASES_USER и YARD_RELEASES_PWD"
    exit 1
fi
...
copy_distr_to_downloads_path() {
  found=1
  found_run_file=1
  echo "Проверяем наличие локальных дистрибутивов в папке /distr"
  case "$installer_type" in
      edt)
          ...
          ;;
      server)
          echo "Проверяем наличие локальных дистрибутивов для сервера 1С"
          ...
          if [ -f "/distr/$file_name_srv" ]; then
            ...
            echo "Локального дистрибутива сервера не найдено в папке distr"
          fi
          ;;
      server32)
  ...
  echo "Завершили проверку локальных дистрибутивов в папке /distr"
  return $found
}
...
check_file() {
  found=1
  echo "Проверяем наличие файлов в каталоге $DOWNLOADS_PATH"
  # Проверяем, появились ли файлы в каталоге
    ...
    echo "Завершили проверку наличия файлов в каталоге $DOWNLOADS_PATH"
    return $found
}
...
echo "Удаляем временные файлы и каталоги"
mkdir -p $DOWNLOADS_PATH
rm -f $DOWNLOADS_PATH/.gitkeep
chmod 777 -R /tmp

# Проверяем, есть ли дистрибутивы локально
echo "Проверяем наличие локальных дистрибутивов"
check_local_distr
local_distr_found=$?
